// ============================================
// MGKEIT Find — общее хранилище и функции
// ============================================

// Хранилище объявлений (изначально пустое)
let items = [];
...
function campusName(c) {
    return c === 'biryulyovo' ? 'Бирюлёво' : c;
}
...
function openModal(id) {
    ...
    document.getElementById('modal-body').innerHTML = `
        ...
        <div class="dev-notice">🔧 <span>Система обратной связи <strong>в разработке</strong>.</span></div>
    `;
...
function handleSubmit(e) {
    ...
    const reward = document.getElementById('item-reward').value;

    if (!type || !title) {
        alert('Заполните обязательные поля: Тип и Название');
        return false;
    }
    ...
    const newItem = {
        ...
        campus,
        title,
        location: location || 'Корпус ' + campusName(campus),
        description: desc,
    ...
    if (preview) {
        preview.innerHTML = `
            <div class="upload-preview">
                <span>🖼️</span>
                <div style="flex:1;"><strong>${esc(file.name)}</strong><br><span style="font-size:11px;color:var(--gray-500);">${(file.size/1048576).toFixed(1)} МБ</span></div>
